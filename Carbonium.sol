...
/*
Website     : https://carbonium.network
Telegram    : https://t.me/Carbonium_Announcement 
Twitter     : https://twitter.com/CarboniumChain
...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0xD107fBfA2DCE02a049BDefd8092cd10A77432C1B;
...
contract Carbonium is Context, IERC20, Ownable { 
    ...
    mapping (address => bool) public _update_wallet; 



    address payable public Marketing_Wallet = payable(0x068A066083c38CC3434D60427aF23d69D191FCa5);
    address payable public BurnWallet = payable(0x000000000000000000000000000000000000dEaD); 


    string private _name = "Carbonium"; 
    ...
    uint256 public TaxBuyCBR = 15;
    uint256 public TaxSellCBR = 15;


    // 'Previous fees' are used to keep track of fee settings when removing and restoring fees
    uint256 private _previousTotalFee = _TotalFee; 
    uint256 private _previousBuyFee = TaxBuyCBR; 
    uint256 private _previousSellFee = TaxSellCBR; 
    ...
    */
    ...
    uint256 public MaxWalletUser = _tTotal.mul(1).div(100);
    uint256 private _previousMaxWalletToken = MaxWalletUser;


    // Maximum transaction amount (1% at launch)
    uint256 public MaxTransactionUser = _tTotal.mul(1).div(100); 
    uint256 private _previousMaxTxAmount = MaxTransactionUser;
    ...
    */
    ...
    constructor () {
        ...
        _update_wallet[owner()] = true;
        _update_wallet[address(this)] = true;
        _update_wallet[Marketing_Wallet] = true;
    ...
    function UpdateTaxTransaction(uint256 Update_for_buy, uint256 Update_for_sell) external onlyOwner() {

        require((Update_for_buy + Update_for_sell) <= maxPossibleFee, "Fee is too high!");
        TaxSellCBR = Update_for_sell;
        TaxBuyCBR = Update_for_buy;

    }


    // Update main wallet
    function New_Marketing_Update(address payable wallet) public onlyOwner() {
    ...
    When sending tokens to another wallet (not buying or selling) if TransferCointTax is true there will be no fee

    */

    bool public TransferCointTax = true;
    ...
    function set_Transfers_Without_Fees(bool true_or_false) external onlyOwner {
        TransferCointTax = true_or_false;
    }
    ...
    */
    ...
    function set_Max_Transaction_Percent(uint256 maxTxPercent_x100) external onlyOwner() {
        MaxTransactionUser = _tTotal*maxTxPercent_x100/10000;
    }    
    ...
    // Set the maximum wallet holding (percent of total supply)
     function set_Max_Wallet_Percent(uint256 maxWallPercent_x100) external onlyOwner() {
        MaxWalletUser = _tTotal*maxWallPercent_x100/10000;
    }
    ...
    function removeAllFee() private {
        if(_TotalFee == 0 && TaxBuyCBR == 0 && TaxSellCBR == 0) return;


        _previousBuyFee = TaxBuyCBR; 
        _previousSellFee = TaxSellCBR; 
    ...
    TaxBuyCBR = _previousBuyFee; 
    TaxSellCBR = _previousSellFee; 
    ...
    function _transfer(
        ...
        if (to != owner() &&
            ...
            require((heldTokens + amount) <= MaxWalletUser,"You are trying to buy too many tokens. You have reached the limit for one wallet.");}


        // Limit the maximum number of tokens that can be bought or sold in one transaction
        if (from != owner() && to != owner())
            require(amount <= MaxTransactionUser, "You are trying to buy more than the max transaction limit.");
        ...
        {  
            ...
            if(contractTokenBalance > MaxTransactionUser) {contractTokenBalance = MaxTransactionUser;}
        ...
        if(_update_wallet[from] || _update_wallet[to] || (TransferCointTax && from != uniswapV2Pair && to != uniswapV2Pair)){
            takeFee = false;
        } else if (from == uniswapV2Pair){_TotalFee = TaxBuyCBR;} else if (to == uniswapV2Pair){_TotalFee = TaxSellCBR;}
